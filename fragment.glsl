...
vec3 shadePhong(in vec3 ray_direction, in vec3 normal, in vec3 dir_to_light, in vec3 incident_intensity, in vec3 diffuse_color) {
    vec3 light_incident;
    float ray_dot_normal = dot(normal, -ray_direction);

    if (ray_dot_normal > 0.0)
        light_incident = incident_intensity * max(0.0, dot(normal, dir_to_light));	        // Brightness depends on the angle of the light and normal

	vec3 specular;
	vec3 light_ray = -dir_to_light;													        // Vector pointing from light to object

	if (dot(normal, dir_to_light) > 0.0) {												    // Only add specular if light is on the same side as normal
		vec3 vect_reflection = light_ray - 2.0 * dot(light_ray, normal) * normal;	        // Reflection vector pointing away from object
		...
		specular = light_incident * specular_color * reflection_intensity;
	}

    return light_incident * diffuse_color + specular;
}

bool intersectSphere(vec3 ray_origin, vec3 ray_direction, vec3 center_, float tmin, inout float t_hit) {
    float radius_ = 0.8;
	...
	float C = dot(tmp, tmp) - radius_*radius_;
...
bool intersectPlane(vec3 ray_origin, vec3 ray_direction, float offset, vec3 normal, float tmin, inout float t_hit) {
    float d = normal.x + normal.y*offset + normal.z;
...
bool intersect(vec3 ray_origin, vec3 ray_direction, inout float t_hit, float tmin, out vec3 position, out vec3 color, out vec3 normal) {
    ...
    for (int i = 0; i < 5; i++) {
	    ...
	    bool tmp = intersectSphere(ray_origin, ray_direction, center, tmin, t_hit);
        if (tmp) {
...
bool intersectShadowRay(vec3 ray_origin, vec3 ray_direction, inout float t_hit, float tmin) {
    for (int i = 0; i < 5; i++) {
        vec3 center = sphereCenters[i];
	    if (intersectSphere(ray_origin, ray_direction, center, tmin, t_hit))
            return true;
    }
    for (int i = 0; i < 1; i++) {
        float planeOffset = planeOffsets[i];
